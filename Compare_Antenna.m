...
A_Size = 0.5; % Aperture size of the transmit region normalized by the wavelength
BCD_Result = zeros(1,length(Antenna_Number)); % The proposed BCD-based method
FPA_Result = zeros(1,length(Antenna_Number)); % The fixed-posistion antenna (FPA)-based method
...
parfor Monte = [1:1:Monte_Carlo]  
    ...
    BCD_Result_Tmp = ones(1,length(Antenna_Number));
    FPA_Result_Tmp = ones(1,length(Antenna_Number));
    for Antenna_Number_Index = [1:1:length(Antenna_Number)]
        [Monte, Antenna_Number_Index]
        M = Antenna_Number(Antenna_Number_Index); % BS Antenna Number
        ...
        BCD_Result_Tmp(Antenna_Number_Index) = Sum_Rate(end);
        ...
        FPA_Result_Tmp(Antenna_Number_Index) = Sum_Rate(end);
    end
